...
def getTemplate(ptType,forForm,metaField) :
    ...
    return {'__ptConfig' : prps,
            ...
            '__ppChoices' : choices,
            '__ppTypes' : ppTypes,
            '__roProperties' : objConfig.roProperties or []
            }

#######################
def defineFormLayout() : 

    def setFieldDefaults(prLayout, key) :
        # Asigna los fieldDefaults q vienen en los contenedores
        sAux = prLayout.get(key)
        if (sAux) :
            prLayout['fieldDefaults'][key] = prLayout[key]


    def defineProtoFormItem(me, parent, protoObj, protoIx) :
        ...
        if (sDataType == dict) :
            ...
            if (not __ptType):
                ...
                if(myFld):                        
                    ...
                    # }
                    #########################A verifer###############################################################
                    prLayout = Ext.apply(template.get('__ptConfig'), protoObj.get('__ptConfig'));
                ...
                #########################A REVOIR###############################################################
                if (_SM.loadPci(protoObj.get('viewCode'), False)): 
                    ...
                    prLayout = {
                                ...
                                'padding' : '4',
                                'border' : 1,
                                'text' : 'ERROR: grid definition not found ' + protoObj.get('viewCode'),#################################                    
                                'style' :{
                                          'borderColor' : 'red',
                                          'borderStyle' : 'solid',
                                          'bodyStyle' : ';border-right:none;border-left:none;border-top:none;'
                    ...
                    ######################################################################################
                    _SM.errorMessage('defineProtoFormItem', protoObj.get('viewCode') + ' not found!!')
                ...
                #Tooltip
                if (prLayout.get('tooltip')) :
                    fun = lambda c : Ext.create('Ext.tip.ToolTip', {'target' : c.getEl(), 'trackMouse' : True, 'html' : prLayout['tooltip']})#####################
                    prLayout['listeners'] = {'render' : fun}
        ...
        ################### A VOIR#############################################################
        #me = this 
        #myFormDefinition, myMeta, ixV, lObj, prItem
        ## @formatter:on

        #myFormDefinition = _SM.clone(self.myMeta['formConfig'])
        #myMeta = self.myMeta
