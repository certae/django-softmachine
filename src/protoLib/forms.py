...
def getFieldConfig(field_name, django_field, value = None):
   ...
   # print field_name, field_class_name
    ...
    # foreignkeys or custom choices
        ...
        config['store'] = "new Ext.data.SimpleStore({fields: ['id','display'],  data : %s })" % ( utilsWeb.JSONserialise(choices))
    ...
    elif field_class_name in ['DateField', 'DateTimeField']:
        config['xtype'] = 'datefield'
        dformat = form_field.input_formats
        if dformat:
            dformat = dformat[0]
        config['format'] = utilsBase.DateFormatConverter(to_extjs = dformat)
        #config['hiddenFormat'] = utilsBase.DateFormatConverter(to_extjs = form_field.input_formats[0])
    ...
    # datetime : use sakis xdatetime ext.ux
    elif field_class_name == 'DateTimeField ERROR':
        ...
        config['timeFormat'] = utilsBase.DateFormatConverter(to_extjs = timef)
        config['timeWidth']  = 60
        config['dateWidth']  = 100
        config['dateFormat'] = utilsBase.DateFormatConverter(to_extjs = datef)
        if date_format:
            config['hiddenFormat'] = utilsBase.DateFormatConverter(to_extjs = date_format)
        if value:
           # config['value'] = value
...
class ExtJsForm(object):
    ...
    def as_extjs(self, excludes = []):
        ...
        return utilsWeb.JSONserialise(config_dict)
