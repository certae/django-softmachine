...
Ext.define('ProtoUL.view.ProtoMasterDetail', {
    ...
    initComponent: function() {
        ...
        this.protoMasterGrid.on({
            selectionChange: {
                fn: function(rowModel, record, rowIndex, eOpts) {
                    if (record) {
    ...
    linkDetail: function() {
        ...
        if (tmpStore.protoMasterId == me.idMasterGrid) {
            return;
        }
        ...
        if (!me.protoMasterGrid.rowData) {
            protoFilter = [{
                "property": pDetail.detailField,
                "filterStmt": -1
            }];
        } else {
            // En caso de q el master no sea el pk
            var rowDataIx = me.idMasterGrid;
            if (pDetail.masterField != 'pk') {
                rowDataIx = me.protoMasterGrid.rowData[pDetail.masterField];
            }
            protoFilter = [{
                "property": pDetail.detailField,
        ...
        if (rowData) {
            myDetGrid.detailTitle = rowData[masterTitleField];
        }
    ...
    setDetDefaults: function(me, myDetGrid) {
        var pDetail = myDetGrid.detailDefinition, rowData = me.protoMasterGrid.rowData, nField = pDetail.detailField.replace(/__pk$/, '_id');
