...
Ext.define('ProtoUL.ux.FieldHtmlEditor', {
	...
	initComponent : function() {
		...
		function getPlugins() {
			return [new ProtoUL.ux.HtmlEditor.Word(), 
...
Ext.define('ProtoUL.ux.HtmlEditor.Word', {
	...
	checkIfPaste : function(e) {
		...
		if (e.V == e.getKey() && e.ctrlKey && this.wordPasteEnabled) {
			...
			var parts = [this.cmp.getValue().substr(0, diffAt), this.fixWordPaste(this.cmp.getValue().substr(diffAt, (this.curLength - this.lastLength))), this.cmp.getValue().substr((this.curLength - this.lastLength) + diffAt, this.curLength)];
