...
Ext.define('ProtoUL.ux.JUtil', {
    ...
    syncContent: function (url, nocache, response, params) {
        ...
        // TODO:  urlEncode   Deprecated since Extjs 4.0 
    ...
    getUrlAction: function (name, defaultValue, url) {
        ...
        if (results == null) return isNaN(defaultValue) ? defaultValue : Number(defaultValue);
        else return isNaN(results[1]) ? results[1] : Number(results[1]);
    },
                ...
                case 'select-multiple':
                    for (var j = 0; j < el.options.length; j++) {
                        if (el.options[j].selected) {
                            data = setData(data, name, Ext.isIE ? (el.options[j].attributes['value'].specified ? el.options[j].value : el.options[j].text) : (el.options[j].hasAttribute('value') ? el.options[j].value : el.options[j].text));
                        }
                    }
                    break;
                case 'radio':
    ...
    require: function (packages, options) {
        var _require = function (packages, options) {
                ...
                for (var i = 0; i < packages.length; i++) {
                    ...
                    var uri = dir + (dir.length != 0 && dir.charAt(dir.length - 1) != '/' ? "/" : "") + url.directory + url.file + (url.query ? '?' + url.query : '');
                    if (url.ext == 'css') {
                        ret.css.push(uri);
